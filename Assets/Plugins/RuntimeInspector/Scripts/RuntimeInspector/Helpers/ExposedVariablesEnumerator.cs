...
namespace RuntimeInspectorNamespace
{
	public class ExposedVariablesEnumerator : IEnumerator<MemberInfo>, IEnumerable<MemberInfo>
	{
		...
		public ExposedVariablesEnumerator(RuntimeInspector runtimeInspector, MemberInfo[] variables, List<VariableSet> hiddenVariables, List<VariableSet> exposedVariables, Visibility fieldVisibility, Visibility propertyVisibility )
		{
			index = -1;
		...
		private bool ShouldExposeVariable( MemberInfo variable )
		{
			...
			if (runtimeInspector.VariableFilter != null)
            {
				if (!runtimeInspector.VariableFilter(variableName))
					return false;
			}
